...
//                        Log.e("UserFound","User FF Found");
    ...
    public void updateSchedule(){
        ...
        ClassScheduleItem scheduleItemm = new ClassScheduleItem();
        scheduleItemm = scheduleItem;

        scheduleItemm.setT_name(sub_teacher);
        scheduleItemm.setSub_name(subName);
        scheduleItemm.setSub_code(sub_code);
        scheduleItemm.setDay(selectedDay);
        scheduleItemm.setRoom(sub_room);
        scheduleItemm.setSection(sub_sec);
        scheduleItemm.setStart_time(from_time);
        scheduleItemm.setEnd_time(to_time);

//        Toast.makeText(getApplicationContext(),""+scheduleItemm.getUniqueId(),Toast.LENGTH_SHORT).show();

        CScheduleD scheduleD = new CScheduleD(database);
        long rs = scheduleD.updateSchedule(scheduleItemm);
        if(rs==-1){
            Toast.makeText(this,"Schedule Data Not updated! Try again!  "+rs,Toast.LENGTH_SHORT).show();
            ...
            Toast.makeText(this,"Schedule Successfully Updated, Thanks!",Toast.LENGTH_SHORT).show();
            finish();
        }
    }
    ...
    private void showTimePickerDialogTwo() {
        ...
        TimePickerDialog timePickerDialog = new TimePickerDialog(EditSchedule.this, android.R.style.Theme_Holo_Dialog_MinWidth ,new TimePickerDialog.OnTimeSetListener() {
            @Override
    ...
    private void showTimePickerDialog() {
        ...
        t1Hour = calendar.get(Calendar.HOUR_OF_DAY);
        t1Minute = calendar.get(Calendar.MINUTE);
        TimePickerDialog timePickerDialog = new TimePickerDialog(EditSchedule.this, new TimePickerDialog.OnTimeSetListener() {
            @Override
            public void onTimeSet(TimePicker view, int hourOfDay, int minute) {
                t1Hour = hourOfDay;
                t1Minute = minute;
