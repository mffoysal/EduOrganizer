...
public class CScheduleD implements UserCallback {
    ...
    public CScheduleD(SQLiteDatabase database) {
        this.database = database;
    }

    public CScheduleD(Context context) {
    ...
    public long insertSchedule(ClassScheduleItem user) {
        long l=-2;
        int r=0;
...
//        values.put(COLUMN_ID, new Admin().uniqueId());
    ...
    public ClassScheduleItem getSchedule(String uniqueId){
        ClassScheduleItem user = new ClassScheduleItem();
    ...
    public List<ClassScheduleItem> getAllSchedule() {
        List<ClassScheduleItem> userList = new ArrayList<>();
    ...
    public List<ClassScheduleItem> getAllSchedule(String userId, String tempCode) {
        List<ClassScheduleItem> userList = new ArrayList<>();

        String selection = "tId = ? AND (temp_code = ? OR temp_num = ?)";

        String[] selectionArgs = new String[]{userId, tempCode, tempCode};
...
//        Cursor cursor = database.query(TABLE_SCHEDULE, null, selection, selectionArgs, null, null, null);
        ...
        String sqlQuery = "SELECT * FROM "+TABLE_SCHEDULE+" " +
                "WHERE tId= ? AND temp_code = ? OR temp_num = ?" +
                "ORDER BY " +
                "CASE " +
        ...
        Cursor cursor = database.rawQuery(sqlQuery, new String[]{userId,tempCode,tempCode,currentTime});
    ...
    public List<ClassScheduleItem> getAllScheduleByRoutine(String tempNum, String tempCode) {
        List<ClassScheduleItem> userList = new ArrayList<>();

        String selection = "temp_num = ? AND (temp_code = ? OR temp_num = ?)";

        String[] selectionArgs = new String[]{tempNum, tempCode, tempCode};
        SimpleDateFormat sdf = new SimpleDateFormat("hh:mm a", Locale.getDefault());
        String currentTime = sdf.format(new Date());
...
//        Cursor cursor = database.query(TABLE_SCHEDULE, null, selection, selectionArgs, null, null, null);
        ...
        String sqlQuery = "SELECT * FROM "+TABLE_SCHEDULE+" " +
                "WHERE temp_num= ? AND temp_code = ? OR temp_num = ?" +
                "ORDER BY " +
                "CASE " +
                "WHEN start_time >= ? THEN 1 " +
                "ELSE 2 " +
        ...
        Cursor cursor = database.rawQuery(sqlQuery, new String[]{tempNum,tempCode,tempCode,currentTime});
    ...
    public int deleteAllScheduleWithTempCode(String temCode) {
        String whereClause = COLUMN_TEMP_NUM + " = ? OR temp_code = ?";
        String[] whereArgs = {temCode,temCode};

        return database.delete(TABLE_SCHEDULE, whereClause, whereArgs);
    }

    public int deleteAllScheduleWithTempCode(String temCode,String tempNum) {
        String whereClause = COLUMN_TEMP_NUM + " = ? OR temp_code = ?";
        String[] whereArgs = {tempNum,temCode};

        return database.delete(TABLE_SCHEDULE, whereClause, whereArgs);
    }

    public List<ClassScheduleItem> getAllSchedule(String day) {
        List<ClassScheduleItem> userList = new ArrayList<>();
//        Cursor cursor = database.rawQuery("SELECT * FROM " +TABLE_SCHEDULE + " WHERE uniqueId = ? OR stdId=? OR tId=? ",new String[]{uniqueId,uniqueId,uniqueId});
...
//        );
    ...
    public List<ClassScheduleItem> getAllSchedule(String userId, String tempCode,String day) {
        List<ClassScheduleItem> userList = new ArrayList<>();
//        Cursor cursor = database.rawQuery("SELECT * FROM " +TABLE_SCHEDULE + " WHERE uniqueId = ? OR stdId=? OR tId=? ",new String[]{uniqueId,uniqueId,uniqueId});

        String selection = "day = ? OR day = ? AND tId = ? AND temp_code = ? OR temp_num = ?";

        String[] selectionArgs = new String[]{day, "Everyday", userId, tempCode, tempCode};
        SimpleDateFormat sdf = new SimpleDateFormat("hh:mm a", Locale.getDefault());
        String currentTime = sdf.format(new Date());
...
//        );
        ...
        String sqlQuery = "SELECT * FROM "+TABLE_SCHEDULE+" " +
                "WHERE (day = ? OR day= ?) AND tId = ? AND (temp_code = ? OR temp_num = ?)" +
                "ORDER BY " +
                "CASE " +
                "WHEN start_time >= ? THEN 1 " +
                "ELSE 2 " +
        ...
        Cursor cursor = database.rawQuery(sqlQuery, new String[]{day,"Everyday",userId,tempCode,tempCode, currentTime});
    ...
    public List<ClassScheduleItem> getAllSchedule(String sId, String tempNum, String tempCode,String day) {
        List<ClassScheduleItem> userList = new ArrayList<>();
//        Cursor cursor = database.rawQuery("SELECT * FROM " +TABLE_SCHEDULE + " WHERE uniqueId = ? OR stdId=? OR tId=? ",new String[]{uniqueId,uniqueId,uniqueId});

        String selection = "day = ? OR day = ? AND sId = ? AND temp_code = ? OR temp_num = ?";

        String[] selectionArgs = new String[]{day, "Everyday", sId, tempCode, tempCode};
        SimpleDateFormat sdf = new SimpleDateFormat("hh:mm a", Locale.getDefault());
        String currentTime = sdf.format(new Date());
...
//        );
        ...
        String sqlQuery = "SELECT * FROM "+TABLE_SCHEDULE+" " +
                "WHERE (day = ? OR day= ?) AND sId = ? AND (temp_code = ? OR temp_num = ?)" +
                "ORDER BY " +
                "CASE " +
                "WHEN start_time >= ? THEN 1 " +
                "ELSE 2 " +
        ...
        Cursor cursor = database.rawQuery(sqlQuery, new String[]{day,"Everyday",sId,tempCode,tempCode, currentTime});
    ...
    public List<ClassScheduleItem> getAllScheduleByRoutine(String sId, String tempCode,String day) {
        List<ClassScheduleItem> userList = new ArrayList<>();
//        Cursor cursor = database.rawQuery("SELECT * FROM " +TABLE_SCHEDULE + " WHERE uniqueId = ? OR stdId=? OR tId=? ",new String[]{uniqueId,uniqueId,uniqueId});
...
//        );
    ...
    public long updateSchedule(ClassScheduleItem u) {
    ...
    public void saveScheduleInformation(String uniqueId, ClassScheduleItem u) {
...
//                    Toast.makeText(this, "Failed to save user information: " + e.getMessage(), Toast.LENGTH_SHORT).show();
    ...
    public void saveScheduleInformationWithRealtime(ClassScheduleItem scheduleItem) {
        // Save additional user information in Firebase Realtime Database
        FirebaseDatabase firebaseDatabase = FirebaseDatabase.getInstance();
...
//                    Toast.makeText(this, "Failed to save user information: " + e.getMessage(), Toast.LENGTH_SHORT).show();
    ...
    public void saveScheduleInformationWithRealtime(String uniqueId, ClassScheduleItem u) {
        // Save additional user information in Firebase Realtime Database
        FirebaseDatabase firebaseDatabase = FirebaseDatabase.getInstance();
        DatabaseReference usersRef = firebaseDatabase.getReference("users");
...
//                    Toast.makeText(this, "Failed to save user information: " + e.getMessage(), Toast.LENGTH_SHORT).show();
    ...
    public List<ClassScheduleItem> getUnsyncedSchedulesFromSQLite() {
        List<ClassScheduleItem> unsyncedUsers = new ArrayList<>();
    ...
    public void checkAndSyncData() {
        List<ClassScheduleItem> unsyncedUsers = getUnsyncedSchedulesFromSQLite();
        for (ClassScheduleItem user : unsyncedUsers) {
            // Add or update user data in Firebase
            addOrUpdateScheduleInFirebase(user);
        }
    }

    public void addOrUpdateScheduleInFirebase(ClassScheduleItem user) {
        DatabaseReference usersRef = FirebaseDatabase.getInstance().getReference().child("classSchedule").child(String.valueOf(user.getId()));
...
//                    Toast.makeText(this, "Failed to sync user: " + e.getMessage(), Toast.LENGTH_SHORT).show();
    ...
    public void updateSyncStatusInSQLite(int userId, int syncStatus) {
        ContentValues values = new ContentValues();
        values.put(ScheduleContract.ScheduleEntry.COLUMN_SYNC_STATUS, syncStatus);

        String selection =  "id = ?";
        String[] selectionArgs = {String.valueOf(userId)};

        database.update(ScheduleContract.ScheduleEntry.TABLE_NAME, values, selection, selectionArgs);
    }


    public void updateScheduleData(ClassScheduleItem user, School school) {
        DatabaseReference usersRef = FirebaseDatabase.getInstance().getReference("classSchedule");
    ...
    public ClassScheduleItem getScheduleData(String userPhone) {
